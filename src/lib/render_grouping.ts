// 汎用リングレンダラ（8×4・9×4 両対応）2026-06-17
//
// /tmp/grouping.json = { strands?, groups:[{name, members:[...], color}] } を勾配順で。
// 全カテゴリが同数(4)なら 案A=案B。新規学部を含む場合は v2判定式を 全active集合で再計算
// （＝診断システムを新編成に合わせて再構築した前提の適応度）。
//
// 実行(JSON準備後):
//  <CA>/node_modules/.bin/tsc <CA>/src/lib/render_grouping.ts --outDir /tmp/rg --module commonjs \
//   --target es2020 --moduleResolution node --esModuleInterop ; node /tmp/rg/render_grouping.js [outprefix]

import { departments, AXIS_COUNT } from "./departments";
import * as fs from "fs";

const OUT = "/tmp/ringdemo";
const PREFIX = process.argv[2] || "grp";
fs.mkdirSync(OUT, { recursive: true });

type Fac = { id: string; name: string; scores: number[]; gradExempt?: boolean };
// 新規5学部（recat_9x4.ts と同じ類推ベクトル）
const NEW: Fac[] = [
...
// ===== v2判定式を active集合で再計算（mixed = 全22軸計測）=====
...
function sampleAt(valNorm: number[], alpha: number): { v: number; color: string } {
  ...
  // コサイン補間: 各制御点(山・谷の頂点)で傾き0になり先端が丸くなる。
  // 値は t=0→lo, t=1→hi のままなので山谷の位置・高さは不変、形状だけ滑らかに。
  const tt = (1 - Math.cos(t * Math.PI)) / 2;
  return { v: valNorm[lo] + (valNorm[hi] - valNorm[lo]) * tt, color: lerpHsl(hsls[lo], hsls[hi], tt) };
}
...
${lines.join("\n")}
${labels.join("\n")}
<text x="${cx}" y="${size - 14}" text-anchor="middle" font-family="sans-serif" font-size="19" fill="#444">${cfg.groups.length}カテゴリ×4 (${NFAC}学部) / ${STRANDS}本 / ペルソナ${persona}</text>
</svg>`;
}

for (const p of ["A", "B", "C"]) fs.writeFileSync(`${OUT}/${PREFIX}_${p}.svg`, render(p));
console.log(`rendered ${PREFIX}_A/B/C.svg  (${cfg.groups.length}群 ${NFAC}学部 ${STRANDS}本)`);
console.log("勾配順:");
cfg.groups.forEach((g, i) => console.log(`  G${i + 1} ${g.name} [${g.color}]: ${g.members.join(", ")}`));

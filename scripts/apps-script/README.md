...
## 新3軸（PURE/BIO/PROC）への移行（2026-06-12 実装・要実行）

フロントに新3軸 **PURE（純粋志向）/ BIO（生体・生命現象）/ PROC（量産・プロセス）** が追加され、
`code.gs` も 19軸 → **22軸** 対応に更新済み。`diagnoses` / 学校別 `students` の各シートで、
新コードは「身体運動」と「回答変更ログ／診断バージョン」の**間に3列**を書き込む。

⚠️ **既存シートをそのままにして本番反映すると、旧行と新行で列がズレる**
（旧行の「回答変更ログ」等が新行の軸値と縦に混在する）。本番 push の前に必ず移行すること。

### 手順（本番 push の前に）

1. Apps Script エディタに更新版 `code.gs` を貼り直す（既存コード全削除→貼り付け）
2. 関数 **`migrateAddNewAxisColumns`** を選択 → 実行
   - マスタ `diagnoses`、`SCHOOL_SHEET_MAP` の全 `students`、自動生成済み学校の `students` に対し、
     「身体運動」の直後へ空の3列（`純粋志向` / `生体・生命現象` / `量産・プロセス`）を挿入する
   - **冪等**：既に「純粋志向」列があるシートはスキップ。何度実行しても二重挿入しない
   - 旧行の新3軸セルは空欄のまま（その診断は新軸を計測していないので正しい）
   - 実行ログ（表示 → ログ）で各シートの結果を確認する
3. **再デプロイ**：デプロイ → 既存デプロイの管理 → 鉛筆 → **新バージョン** → デプロイ（URL 不変）
4. 本番（ring-map.com）を理系/混合で1回完走させ、`diagnoses` の新3軸列に値が入ることを確認

> `satisfactions` / `feedback` は軸列を持たないので移行不要。
...
`students` / `feedback` シートは、追記後に自動で **学年 → クラス → 出席番号** 順にソートされる
（`sortSchoolSheet()` が `appendRow` の直後に呼ばれる）。教員がクラス順に上から見られる状態を維持するため。

出席番号は数値として保存される（`toNumberOrKeep()`）ので、1, 2, 10, 22 のような正しい数値順で並ぶ。

## 配布資料のスクショ用デモデータ

エキスポ等で配るリーフレットに「実際のスプレッドシート画面」を載せたい時：

1. Apps Script エディタで `populateDemoData` を選択 → 実行
   - 母校（`D114310000160`）の `students` / `feedback` に約15行のリアルな見栄えのデモデータが投入される
   - `session_id` は `DEMO_YYYYMMDD_NN` 形式で統一されるので識別容易
2. 母校 Spreadsheet を開いて、スクショ撮影
3. 撮影が終わったら `clearDemoData` を実行 → `DEMO_` プレフィックスの行が一括削除される

本番運用が始まる前のみ使う想定。本番開始後はもう実データが入るので不要。
